...
import { Color, ColoredLetter, countOccurrence, DoubleColoredLetter, getStrongestColor, initColoredArrayOfLetters, setColorOfLetter } from "./color"
import { getLetter, updateDoubleColoredKeyboard, updateKeyboard } from "./keyboard"
import { updateWordMatrix, WordMatrixWithID } from "./wordmatrix"

/**********************************************/
// WORDLE COLOR FUNCTIONS
/**********************************************/
...
/**********************************************/
// DORDLE COMPARE FUNCTIONS
/**********************************************/

export type DorldeGameElements = {
  wordMatrixLeft: WordMatrixWithID,
  wordMatrixRight: WordMatrixWithID,
  keyboard: DoubleColoredLetter[]
}

export const updateDordleGameElements = (elements: DorldeGameElements, currentWordLeft: string, currentWordRight: string, currentGuess: string, currentRow: number, guessed: DordleGuessSide): DorldeGameElements => {
  if (currentGuess.length !== 5)
    throw new RangeError("New word in matrix must be of length 5!");

  const coloredGuessLeft = getColoredGuess(Array.from(currentGuess), Array.from(currentWordLeft))
  const coloredGuessRight = getColoredGuess(Array.from(currentGuess), Array.from(currentWordRight))

  return {
    ...elements,
    wordMatrixLeft: { matrixId: elements.wordMatrixLeft.matrixId, matrix: updateWordMatrix(elements.wordMatrixLeft.matrix, coloredGuessLeft, currentRow) },
    wordMatrixRight: { matrixId: elements.wordMatrixRight.matrixId, matrix: updateWordMatrix(elements.wordMatrixRight.matrix, coloredGuessRight, currentRow) },
    keyboard: updateDoubleColoredKeyboard(elements.keyboard, coloredGuessLeft, coloredGuessRight, guessed)
  }
}